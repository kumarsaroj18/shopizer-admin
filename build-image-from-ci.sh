#!/bin/bash
# Build Shopizer Admin Docker image using a pre-built artifact from GitHub Actions CI
#
# Usage:
...
#   [image-tag]      Docker image tag (default: shopizer-admin:ci-latest)
#
# Options:
#   --help, -h       Show this help message
#
# Examples:
#   ./build-image-from-ci.sh kumarsaroj18 shopizer-admin
#   ./build-image-from-ci.sh kumarsaroj18 shopizer-admin main shopizer-admin:1.0.0
#   ./build-image-from-ci.sh kumarsaroj18 shopizer-admin develop shopizer-admin:dev
#
# Prerequisites:
...
IMAGE_TAG="${4:-shopizer-admin:ci-latest}"
...
echo "📦 Building Shopizer Admin Docker image from CI artifact"
...
ARTIFACT_ID=$(gh api \
  ...
  --jq '.artifacts[] | select(.name | startswith("shopizer-admin-build-")) | .id' \
  | head -1)

if [ -z "$ARTIFACT_ID" ]; then
    echo "❌ No artifact starting with 'shopizer-admin-build-' found"
...
if [ "$CACHED_ID" = "$ARTIFACT_ID" ] && [ -d "${DOWNLOAD_DIR}/shopizer-admin/dist" ] && [ -n "$(ls -A "${DOWNLOAD_DIR}/shopizer-admin/dist" 2>/dev/null)" ]; then
    ...
    rm -f "$ARTIFACT_ZIP"
    rm -rf "${DOWNLOAD_DIR}/shopizer-admin" "${DOWNLOAD_DIR}"/*.tar.gz
    ...
    # The artifact zip contains a shopizer-admin-*.tar.gz — extract into shopizer-admin/dist/
    TARBALL=$(find "$DOWNLOAD_DIR" -maxdepth 1 -name "shopizer-admin-*.tar.gz" | head -1)
    if [ -z "$TARBALL" ]; then
        echo "❌ No shopizer-admin-*.tar.gz found in downloaded artifact"
        ls -la "$DOWNLOAD_DIR"
        rm -rf "$DOWNLOAD_DIR"
        exit 1
    fi

    echo "📂 Extracting Angular dist from $(basename "$TARBALL")..."
    mkdir -p "${DOWNLOAD_DIR}/shopizer-admin/dist"
    tar -xzf "$TARBALL" -C "${DOWNLOAD_DIR}/shopizer-admin/dist"
    rm -f "$TARBALL"
...
echo "   Dist ready at: ${DOWNLOAD_DIR}/shopizer-admin/dist/"
...
echo ""
echo "🧹 Cleaning up..."
# Remove dist (large) but keep the cache file so the next run can skip the download
rm -rf "${DOWNLOAD_DIR}/shopizer-admin"

echo ""
echo "✅ Docker image built successfully: $IMAGE_TAG"
echo ""
echo "Run with:"
echo "  docker run -p 4200:80 \\"
echo "    -e APP_BASE_URL=http://localhost:8080/api/v1 \\"
echo "    $IMAGE_TAG"
